...
export async function createAdminHandler(bot) {
  await initAdminTables();

  return async function handleAdminRequest(req, res, url) {
    ...
    if (path.startsWith('/admin/api/')) {
      ...
      if (path === '/admin/api/reports/export' && req.method === 'GET') {
        return secure(async () => {
          ...
          res.writeHead(200, {
            ...
            'Content-Disposition': `attachment; filename="report_${telegramId}_${from}_${to}.xlsx"`
          });
      ...
      if (path === '/admin/api/broadcast' && req.method === 'POST') {
        return secure(async () => {
          ...
          for (const target of targets) {
            try {
              await bot.sendMessage(target, body.text || '');
              stat.sent += 1;
            } catch {
              stat.failed += 1;
            }
      ...
      if (path === '/admin/api/activity-log' && req.method === 'GET') {
        return secure(async () => {
          try {
            const { rows } = await pool.query('SELECT id, action, details, ok, created_at FROM admin_activity_log ORDER BY created_at DESC LIMIT 300');
            return json(res, 200, { items: rows });
          } catch {
      ...
      if (path === '/admin/api/settings' && req.method === 'GET') {
        return secure(async () => json(res, 200, {
          language: 'ru',
          timezone: process.env.TZ || 'UTC',
          currency: 'USD',
          webAppUrl: cfg.webUrl,
